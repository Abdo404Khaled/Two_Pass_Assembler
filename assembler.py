class Assembler(object):
    ...
    def __first_pass(self) -> None:
        ...
        __lineC = 0
        while not __ended:
            __line = self.__asm[__lc]
            if self.__islabel(format(__line[0])):
                self.__address_symbol_table[__line[0].strip(',')] = self.__format2bin(str(__lineC), 'dec', 12)
                __lc += 1
                __lineC += 1
            elif __line[0].lower() == "org":
                __lineC = int(self.__format2bin(__line[1], 'hex', 12), 2)
                __lc += 1
            elif __line[0].lower() == "end":
                __ended = True
            else:
                __lc += 1
                __lineC += 1
    ...
    def __second_pass(self) -> None:
        ...
        while not __ended:
            __line = self.__asm[__lc]
            # pseudo instructions ****************************************************************
            if __line[0].lower() == "org":
                __lineC = int(self.__format2bin(__line[1], 'hex', 12), 2)
                __lc += 1
            elif __line[0].lower() == "end":
                __ended = True
            elif self.__islabel(format(__line[0])):
                self.__Mri_Func(1, __line, __lineC) if __line[1].lower() in self.__mri_table else ""
                self.__Rri_Func(1, __line, __lineC) if __line[1].lower() in self.__rri_table else ""
                self.__Ioi_Func(1, __line, __lineC) if __line[1].lower() in self.__ioi_table else ""
                self.__labelConv(0, __line, __lineC, 'dec') if __line[1].lower() == "dec" else ""
                self.__labelConv(0, __line, __lineC, 'hex') if __line[1].lower() == "hex" else ""
                __lc += 1
                __lineC += 1
            # instructions ****************************************************************
            elif __line[0].lower() in dict(self.__mri_table.items() | self.__rri_table.items() | self.__ioi_table.items()):
                self.__Mri_Func(0, __line, __lineC) if __line[0].lower() in self.__mri_table else ""
                self.__Rri_Func(0, __line, __lineC) if __line[0].lower() in self.__rri_table else ""
                self.__Ioi_Func(0, __line, __lineC) if __line[0].lower() in self.__ioi_table else ""
                __lc += 1
                __lineC += 1
            # ERROR **********************************************************************************
            else:
                print(f"ERROR in Location 0x{__lineC}")
                __lc += 1
                __lineC += 1

    def __Mri_Func(self, start , __line, __lineC):
        try:
            b0 = '1' if __line[start + 2].lower() == 'i' else '0'
        except IndexError:
            b0 = '0'
        __opCode = self.__mri_table[__line[start].lower()]
        __addr = self.__address_symbol_table[__line[start + 1]]
        __location = self.__format2bin(str(__lineC), 'dec', 12)
        self.__bin[__location] = b0 + __opCode + __addr

    def __Rri_Func(self, start, __line, __lineC):
        __opCode = self.__rri_table[__line[start].lower()]
        __location = self.__format2bin(str(__lineC), 'dec', 12)
        self.__bin[__location] = __opCode

    def __Ioi_Func(self, start, __line, __lineC):
        __opCode = self.__ioi_table[__line[start].lower()]
        __location = self.__format2bin(str(__lineC), 'dec', 12)
        self.__bin[__location] = __opCode

    def __labelConv(self, start, __line, __lineC, __type):
        __operand = self.__format2bin(str(__line[start + 2]), __type, 16)
        __location = self.__format2bin(str(__lineC), 'dec', 12)
        self.__bin[__location] = __operand
